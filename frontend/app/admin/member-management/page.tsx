...
export default function MemberManagement() {
  ...
  const handleDelete = async (member: Member) => {
    if (!confirm(`정말로 ${member.name}님을 삭제하시겠습니까?`)) return
          ...
          <div key={member.id} className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow">
            ...
            <div className="space-y-2">
              ...
              <div className="text-sm text-gray-600 space-y-1">
                ...
                <p><span className="font-medium">학번:</span> {member.profile?.studentId || '미설정'}</p>
                <p><span className="font-medium">학과:</span> {member.profile?.department || '미설정'}</p>
                <p><span className="font-medium">학년:</span> {member.profile?.year || '미설정'}</p>
                <p><span className="font-medium">역할:</span> {member.profile?.position || '부원'}</p>
                <p><span className="font-medium">권한:</span> 
                ...
                <p><span className="font-medium">공개 여부:</span> 
                  <span className={`ml-1 px-2 py-1 rounded text-xs ${
                    member.profile?.isPublic ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                  }`}>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-2xl max-h-screen overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">동아리 역할</label>
                <select
            ...
            <div className="mt-4">
              <label className="flex items-center">
                ...
                />
                <span className="ml-2 text-sm text-gray-700">공개 프로필로 설정</span>
              </label>
            </div>
